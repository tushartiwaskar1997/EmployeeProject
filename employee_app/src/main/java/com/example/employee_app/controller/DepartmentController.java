...
public class DepartmentController {
    ...
    public ResponseEntity<Object> SaveTheDeptDetails(@RequestBody DepartmentRequestDto Deptdetails) throws JsonProcessingException {
        ...
        if (checkvalidation == null) {
            return ResponseHandler.createResponse(MessageConfig.SUCCESS_DEPARTMENT_ADDED, HttpStatus.OK, departmentService.AddTheDepartment(Deptdetails));
        }
    ...
    public ResponseEntity<Object> UpdateTheDepartmentDetails(@RequestBody DepartmentRequestDto departmentDetails) {
        ...
        if (deptoptional.isPresent()) {
            DepartmentDetails departmentDetails1 = deptoptional.get();
            if (!departmentDetails1.getDepartmentName().equals(departmentDetails.getDepartmentName().toLowerCase())) {
                Optional<DepartmentDetails> CheckIfDepartmentPresent = departmentService.CheckIfDepartmentIsPresent(departmentDetails.getDepartmentName().toLowerCase());
                if (CheckIfDepartmentPresent.isPresent()) {
                    return ResponseHandler.createResponse(MessageConfig.ERROR_DEPARTMENT_ALREADY_EXIST, HttpStatus.NOT_ACCEPTABLE, null);
                }
            }
            return ResponseHandler.createResponse(MessageConfig.SUCCESS_DEPARTMENT_UPDATED, HttpStatus.OK, departmentService.UpdateTheDepartment(departmentDetails));
        }
